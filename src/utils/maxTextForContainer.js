export const maxTextForContainer = (container, textString) => {
  ...
  if (container !== null && newText.length >= 248) {
    ...
    const containerHeight = parseFloat(
        containerStyle.height.slice(0, containerStyle.height.length - 2)
    );

    const containerWidth = parseFloat(
        containerStyle.width.slice(0, containerStyle.width.length - 2)
    );

    const containerFontSize = parseFloat(
        containerStyle.fontSize.slice(0, containerStyle.fontSize.length - 2)
    );
